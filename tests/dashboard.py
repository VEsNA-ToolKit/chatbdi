...
def update_csv_data( ) -> None:
	...
	gen_df.to_csv( 'tests/data/gen.csv', mode='w+' )
	emb_df.to_csv( 'tests/data/emb.csv', mode='w+' )
...
def analyse_emb():
	...
	st.markdown( """
		...
		""")
	df : pd.DataFrame = pd.read_csv( 'tests/data/emb.csv' )
...
def analyse_gen( ):
	...
	st.markdown( """
		...
		""")
	df : pd.DataFrame = pd.read_csv( 'tests/data/gen.csv' )
...
def main() -> None :
	...
	st.title( "ChatBDI: _NL2KQML_ Test Results" )
	with st.sidebar:
	...
	st.markdown("""
		...
		""")
	st.markdown( """
		ChatBDI is a tool that allows agents written in Jason to interact with a user using Natural Language.
	 	Our instance of ChatBDI makes use of LLMs to translate the messages written in Natural Language from the user into logical terms understandable from a Jason agent, and in particular into a KQML message.
	  	A KQML message is composed by:
		""")
	st.markdown( """
		- a recipient: who will receive the message. This has been simply handled using the classic "@" (e.g. `@bob` );
		- a performative: the performatives can be many and describe "why" the sender sent the message, we restrict ourselves to `tell`, `askOne` and `askAll`;
		- a content: a logical term understandable from the other agent.
		""")

	st.markdown( """
		### The Flow
		The flow of our instance of ChatBDI is the following:
		1. the user sends a message in Natural Language;
		2. the message is processed by an LLM that extracts the performative from the sentence;
		3. the message is processed by an embedding model that finds the nearest term in the agent's belief base;
		4. the message is processed by a generative model that, given the input sentence and the nearest term as template, generates a logical term as content of the KQML message;
		5. the KQML message is sent to the agent.
	""")
	st.markdown("""
		### The Tests
		Before using ChatBDI in a real scenario, we wanted to test how well LLMs can perform the translation from Natural Language to KQML.
		We created a set of different domains, each one with its own set of terms and examples of sentences with the expected KQML message.
		We then created a set of tests to evaluate the performance of different LLMs in performing the translation.
	""")
	st.markdown( """
		To perform the tests we used Ollama.
		We subdivided the tests into three main categories:
		- embedding finding;
	...
	with col2.container( border=True ):
		st.markdown("""
			##### Performative Extraction
			Given the input sentence, we extract the performative from the sentence using a generative model.
		""")
	...
	for i, domain in enumerate( domains ):
		...
		col1.markdown( f"#### Input and expected results" )
		col2.markdown( f"#### Term Set" )
		col1.dataframe( pd.read_csv( f"tests/input/{domain}/data.csv", delimiter=";") )
		col2.dataframe( pd.read_csv( f"tests/input/{domain}/literals.csv", delimiter=";") )
	st.markdown( """
		Tests have been performed on the following machine:
	""")
	pc_col1, _ = st.columns( [1, 2] )
	pc = pc_col1.container( border=True)
	pc.markdown( "#### 💻 Testing Machine" )
	pc.markdown( """
	...
	""")
	st.markdown( "Now we can proceed with the analysis of the results.")
